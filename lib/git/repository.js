...
const GIT_ARCHIVE_LIMIT_BYTES = config.get('codedb.git_archive_limit');
...
class Repository {
  ...
  // todo [akamel] move out of here
  is_built(options = {}) {
    var { sha } = options;

    return new Make(this.remote()).is_built(this, { sha });
  }

  build(options = {}) {
    var { token, branch, tailf, force } = options;

    return this
            .sha_ify(branch)
            .then((sha) => {
              let dirname = Make.path(this, { sha });

              return this
                      .archive_to_dir({ sha, dirname })
                      .then((dirname) => {
                        winston.info(`repository:snapshot make`, this.remote());

                        return new Make(this.remote()).build(this, dirname, { sha, tailf });
                      });
            });
  }
  ...
  snapshot(options = {}) {
    var { token, branch, ifnonmatch, tailf } = options;
    ...
    return this
            ...
            .then((sha) => {
              ...
              return Promise
                      ...
                      .then(() => {
                        // todo [akamel] it is possible we return a 'make' gzip even if we didn't ask for make
                        winston.info(`repository:snapshot found`, filename);
                      ...
                      .catch((err) => {
                        ...
                        return this
                                ...
                                .then((tar_stream) => {
                                  if (!options.make) {
                                    winston.info(`repository:snapshot no-make`, this.remote());
                                    return tar_stream;
                                  }
                                  ...
                                  return Make
                                          ...
                                          .then((should_build) => {
                                            if (should_build) {
                                              ...
                                              return new Make(this.remote()).stream(this, tar_stream, { sha, ifnonmatch, tailf });
                                            }

                                            winston.info(`repository:snapshot make skipped`, this.remote());
                                            return tar_stream;
                                          });
                                })
                                .then((tar_stream) => {
  ...
  archive_to_dir(options = {}) {
    let { sha, tailf, dirname } = options
      , cwd                     = path.join(this.path, '/.git')
      , abs_dirname             = path.resolve(dirname)
      ;

    return Promise
            .fromCallback((cb) => fs.emptyDir(dirname, cb))
            .then(() => {
              return Log.open(tailf);
            })
            .then((log) => {
              let cmd     = `git archive --format=tar ${sha} | tar xvC ${abs_dirname}`
                , child   = shell.exec(cmd, { async : true, cwd, silent : true })
                ;

              return log.pipe(child);
            })
            .then(() => {
              let filename = path.resolve(dirname, '.git/config')
                , data =
`
[remote "origin"]
	url = ${this.remote()}
...
`
                ;
              return Promise.fromCallback((cb) => fs.outputFile(filename, data, cb));
            })
            .then(() => {
              let filename = path.resolve(dirname, '.git/HEAD')
                , data = `${sha}`
                ;
              return Promise.fromCallback((cb) => fs.outputFile(filename, data, cb));
            })
  ...
  archive(options = {}) {
    return Promise
            .try(() => {
              // todo [akamel] throw if not <sha>
              let { sha } = options;

              let dirname = path.join(this.path, '/.git')
                , args    = ['archive', '--format=tar.gz', sha]
                ;

              return Promise
                      .try(() => {
                        let ret = git_spawn(dirname, args, GIT_ARCHIVE_LIMIT_BYTES);

                        let mid = through2((chunk, enc, callback) => callback(undefined, chunk));

                        ret.on('data', (d) => mid.write(d));
                        ret.on('end', (d) => {
                          // if (d) {
                          //   mid.write(d);
                          // }
                        ...
                        ret.on('error', (err) => mid.dirnameroy(err));

                        return mid;
                      })
                      .tap(() => {
                        winston.info(`repository:archive ok`, this.remote(), dirname);
                      });
            });
  }
