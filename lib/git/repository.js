...
class Repository {
  ...
  acl(criteria) {
    return this
            .getMetadata()
            .then((m) => {
              if (!_.isObject(m) || (m.private && criteria.private === false)) {
                  ...
                  return Promise
                          ...
                          .tap(() => {
                            var query     = { remote : this.remote }
                              , data      = { remote : this.remote, private : false }
                              , options   = { upsert: true }
                              ;

                            return Promise.promisify(db.update, { context : db })(query, data, options);
                          })
                          .catch({ message : 'authentication required but no callback set' }, (err) => {
                            if (options.token) {
                              ...
                              return Promise
                                      ...
                                      .tap(() => {
                                        var query     = { remote : this.remote }
                                          , data      = { remote : this.remote, private : true }
                                          , options   = { upsert: true }
                                          ;
