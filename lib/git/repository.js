...
class Repository {
                      ...
                      .then((token) => {
                        if (token) {
                          ...
                          return github
                                  ...
                                  .then(() => {
                                    return github.create_hook(this, { token });
                                  })
                                  .then((result) => {
                                    winston.info('hook', this.remote(), result);
                                    // todo [akamel] race condition where we create multiple hooks
                                    return this.hset('githook', JSON.stringify(result));
                                  });
                        }
                      });
            });
  ...
  notify(msg = {}) {
    this
      .hook()
      .catch((err) => {
        winston.error('hook:error', this.remote());
      });
