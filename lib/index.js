...
app.post('/sha', jwt_token, (req, res, next) => {
  ...
  Repository
    .get(remote)
    .then((repo) => {
      return repo.sha_ify(branch, { pull : true, token })
    })
    .then((sha) => {
      res.send({
        sha : sha
...
app.post('/blob', jwt_token, (req, res, next) => {
                ...
                return repo
                        ...
                        .then((stat) => {
                          ...
                          if (stat.rawsize > limit) {
                            winston.info(`file is larger than ${limit} limit: ${filename}`);
                            
                            throw new Error('file is larger than ${limit} limit');
                          }
...
app.post('/blob/hotreload', jwt_token, (req, res, next) => {
  var remote          = req.body.remote   //|| 'https://github.com/a7medkamel/taskmill-core-agent.git'
  ...
  // winston.debug('/blob/metadata', remote, branch, filename, token, populate);
    ...
    .then((repo) => {
      return repo
              ...
              .spread((at, repository) => {
                ...
                let meta = blob_metadata(content, populate);
...
app.post('/pull', jwt_token, (req, res, next) => {
...
app.post('/archive', jwt_token, (req, res, next) => {
  var remote          = req.body.remote
    , token           = req.body.token
    , branch          = req.body.branch
    , make            = req.body.make
...
app.post('/ls', jwt_token, (req, res, next) => {
  var remote          = req.body.remote
    , token           = req.body.token
    , branch          = req.body.branch
    , populate        = req.body.populate
