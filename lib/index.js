      ...
      return repo
              ...
              .then((stat) => {
                ...
                return Promise
                        ...
                        .spread((text, at, metadata) => {
                          let item = {
                              stat        : stat
                            , repository  : _.omit(metadata, '_id', 'token')
                          ...
                          // todo [akamel], why do we populate metadata twice, both 'metadata' and 'repository'
                          // if (populate.metadata) {
                          //   _.extend(item, { metadata });
                          // }
...
app.post('/pull', jwt.token, (req, res, next) => {
  ...
  Repository
    ...
    .then((repository) => {
      res.send(_.extend({ message : 'OK', repository }));
    })
    .catch((err) => {
      winston.error(err);
      res.status(400).send({ message : err.message });
